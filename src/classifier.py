...
class Classifier(nn.Module):
    ...
    def __init__(self, cfg, mode='distill'):
#         mode = 'distill'
        self.mode = mode
#         if mode == 'distill':
#             cfg.num_classes *= 2
...
#         feat_map.requires_grad=True
        ...
        for index, num_class in enumerate(self.cfg.num_classes):
            ...
            if self.cfg.fc_bn:
                ...
                feat1 = bn(feat)
                feat2 = F.dropout(feat1, p=self.cfg.fc_drop, training=self.training)
                
            else:
                feat2 = F.dropout(feat, p=self.cfg.fc_drop, training=self.training)
                
                
            logit = classifier(feat2)
            # (N, num_class, 1, 1)
            
#             if index == 0:
                
#                 lna = classifier(feat)
# #                 lna = lna.squeeze(-1).squeeze(-1)
            
#             # (N, num_class)
#             if index == 1:
#                 lnb = classifier(feat).clone()
# #                 lnb = lnb.squeeze(-1).squeeze(-1)
